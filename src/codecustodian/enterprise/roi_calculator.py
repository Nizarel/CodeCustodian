...
class ROICalculator:
    ...
    def generate_report(self, period: str | None = None) -> ROIReport:
        ...
        total_cost = sum(e.ai_cost_usd for e in entries)
        total_hours = sum(
            e.estimated_manual_hours for e in entries if e.was_successful
        )
        ...
        for entry in entries:
            ...
            bucket = by_type.setdefault(ft, {"cost": 0.0, "hours": 0.0, "count": 0})
            bucket["cost"] += entry.ai_cost_usd
            bucket["hours"] += entry.estimated_manual_hours
            bucket["count"] += 1
